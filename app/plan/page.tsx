    ...
    <>
      <section className="flex flex-col items-center sm:items-start justify-center gap-4 py-12 my-3">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full">
          {plans.map((item, index) =><PlanCard key={index} {...item} />)}
        </div>
      </section>
      <Divider />
      <section className="flex flex-col items-center sm:items-start gap-4 py-12 my-3">
        ...
        >
          {faqsR.map((item, index) => <AccordionItem key={index} aria-label={item.title} title={item.title}>
            {item.content}
          </AccordionItem>)}
        </Accordion>
        <Accordion 
        ...
        itemClasses={{title:"title-tiny", content:"text-summary"}}
        >
          {faqL.map((item, index) => <AccordionItem key={index} aria-label={item.title} title={item.title}>
            {item.content}
          </AccordionItem>)}
        </Accordion>
        </div>
