...
class DropdownboxManager {
    ...
    private iconText: fabric.Text = new fabric.Text("Dropdown");
    private border: fabric.Rect;
    private setShowSettingForm: React.Dispatch<React.SetStateAction<any>>;
    private setShowListForm: React.Dispatch<React.SetStateAction<any>>;
    ...
    private dropdownItems: string[] = ['1','2'];
    ...
    constructor(
        ...
        this.setShowListForm = setShowListForm;
    ...
    private trackIconGroup() {
        ...
        this.iconBorder.on('modified', () => {
            ...
            this.svgGroup.set({
              left: this.iconBorder.left! + (this.iconBorder.getScaledWidth() - 100) / 2,
    ...
    private closeShowSettingForm() {
        const groupPosition = this.signMode || this.onlyMyself ? this.iconBorder.getBoundingRect() : this.svgGroup.getBoundingRect();
        this.setShowSettingForm({
            uid: this.uid,
            show: false,
            position: {
    ...
    private showShowListForm() {
        const groupPosition = this.signMode || this.onlyMyself ? this.textbox.getBoundingRect() : this.iconBorder.getBoundingRect();
        this.setShowListForm({
          uid: this.uid,
          show: true,
          position: {
              left: groupPosition.left, // Position horizontally below the group
